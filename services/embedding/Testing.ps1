...
$ServiceUrl = "http://127.0.0.1:8030"
...
function Get-Embedding($Text, [bool]$Normalize = $true) {
    ...
    # Normalize service payloads to a stable shape for all checks.
    $embedding = $null
    $dimensions = $null
    $model = $null
    $metadata = $null
    ...
    if ($null -ne $response.item) {
        if ($null -ne $response.item.embedding) { $embedding = $response.item.embedding }
        elseif ($null -ne $response.item.vector) { $embedding = $response.item.vector }
        $dimensions = $response.item.dimensions
        $model = $response.item.model
        $metadata = $response.item.metadata
    } else {
        if ($null -ne $response.embedding) { $embedding = $response.embedding }
        elseif ($null -ne $response.vector) { $embedding = $response.vector }
        $dimensions = $response.dimensions
        $model = $response.model
        $metadata = $response.metadata
    }

    if ($null -eq $dimensions -and $null -ne $embedding) {
