...
static void on_chat_stream_progress(const char *stage, const char *message, const char *context_mode, gpointer user_data);
...
static void
on_chat_stream_progress(const char *stage, const char *message, const char *context_mode, gpointer user_data)
{
    LiaraWindow *ui = user_data;
    ...
    // Display progress with memory effect indicator if detected
    if (g_strcmp0(stage, "memory_effect_detected") == 0) {
        status_text = g_strdup_printf(
            "✨ Memory Effect Detected\nContext Mode: %s\n%s",
            context_mode != NULL ? context_mode : "MEMORY",
            message != NULL ? message : ""
        );
    } else {
        status_text = g_strdup_printf(
            "Progress: %s\n%s",
            stage != NULL ? stage : "processing",
            message != NULL ? message : ""
        );
    }

    set_text_view_text(ui->status_output, status_text);
}
