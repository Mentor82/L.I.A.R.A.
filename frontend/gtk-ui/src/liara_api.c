...
dispatch_stream_progress_idle(gpointer user_data)
{
    ...
    dispatch->callback(dispatch->stage, dispatch->message, dispatch->context_mode, dispatch->user_data);
    g_clear_pointer(&dispatch->stage, g_free);
    g_clear_pointer(&dispatch->message, g_free);
    g_clear_pointer(&dispatch->context_mode, g_free);
    g_free(dispatch);
    return G_SOURCE_REMOVE;
}

static void
dispatch_stream_progress(StreamRequestContext *context, const char *stage, const char *message, const char *context_mode)
{
    ...
    dispatch->stage = g_strdup(stage != NULL ? stage : "");
    dispatch->message = g_strdup(message != NULL ? message : "");
    dispatch->context_mode = g_strdup(context_mode != NULL ? context_mode : "");
    g_idle_add(dispatch_stream_progress_idle, dispatch);
}
...
dispatch_stream_chunk(StreamRequestContext *context, const char *chunk_text)
{
    ...
    g_idle_add(dispatch_stream_chunk_idle, dispatch);
}
...
dispatch_stream_complete(StreamRequestContext *context, const char *final_payload, GError *error)
{
    ...
    g_idle_add(dispatch_stream_complete_idle, dispatch);
}
...
dispatch_stream_heartbeat(StreamRequestContext *context, const char *heartbeat_payload)
{
    ...
    g_idle_add(dispatch_stream_heartbeat_idle, dispatch);
}
...
stream_request_thread(gpointer user_data)
{
    ...
    while (TRUE) {
        ...
        if (g_strcmp0(current_event, "progress") == 0) {
            // Parse progress event for memory effect detection
            const char *data_str = g_strstrip(line + 5);
            g_autoptr(JsonParser) parser = json_parser_new();
            if (json_parser_load_from_data(parser, data_str, -1, NULL)) {
                JsonNode *root = json_parser_get_root(parser);
                if (JSON_NODE_HOLDS_OBJECT(root)) {
                    JsonObject *obj = json_node_get_object(root);
                    const char *stage = json_object_get_string_member_with_default(obj, "stage", "");
                    const char *message = json_object_get_string_member_with_default(obj, "message", "");
                    JsonObject *metadata = json_object_get_object_member(obj, "metadata");
                    const char *context_mode = "";
                    if (metadata != NULL) {
                        context_mode = json_object_get_string_member_with_default(metadata, "context_mode", "");
                    }
                    dispatch_stream_progress(context, stage, message, context_mode);
                }
            }
            continue;
        }
...
liara_api_post_chat(
    ...
    gpointer user_data)
{
    g_autofree char *body = build_chat_body(session_id, user_id, message, max_tokens);
    ...
    gpointer user_data)
{
    ...
    context->user_data = user_data;
    context->payload = build_chat_body(session_id, user_id, message, max_tokens);
