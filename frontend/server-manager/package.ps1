...
Set-Content -Path (Join-Path $configPath "server-manager.json") -Value "{`n  `"autostart`": false,`n  `"restart_on_nonzero`": false,`n  `"start_delay_ms`": 1500,`n  `"env_file`": `"C:\\ai\\LIARA\\.env`",`n  `"log_level`": `"INFO`"`n}" -Encoding ASCII
